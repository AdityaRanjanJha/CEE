...
CREATE VIEW exam_details_enrolled AS
SELECT e.exam_id, e.exam_name, e.course_id, c.course_name, p.name AS participant_name
FROM EXAM e
INNER JOIN COURSE c ON e.course_id = c.course_id
LEFT JOIN EXAM_PARTICIPANT ep ON e.exam_id = ep.exam_id
LEFT JOIN PARTICIPANT p ON ep.participant_id = p.participant_id;


CREATE TRIGGER update_score_after_grading 
AFTER UPDATE ON RESULT (grading_status, final_score)
FOR EACH ROW
BEGIN
  IF NEW.grading_status = 'GRADED' THEN  -- Update only after grading
    UPDATE RESULT r
    SET score = NEW.final_score
    WHERE r.result_id = OLD.result_id;
  END IF;
END;

CREATE TRIGGER flag_unanswered_questions 
AFTER UPDATE ON EXAM_PARTICIPANT (submitted_at)
FOR EACH ROW
BEGIN
  DECLARE missing_answers INT;
  
  -- Logic to count unanswered questions for the participant's exam
  
  IF missing_answers > 0 THEN
    UPDATE EXAM_PARTICIPANT_ANSWER epa
    SET answer_id = NULL
    FROM EXAM_PARTICIPANT ep
    WHERE epa.participant_id = ep.participant_id
    AND epa.exam_id = ep.exam_id
    AND epa.answer_id IS NULL;
  END IF;
END; CREATE TRIGGER flag_unanswered_questions 
AFTER UPDATE ON EXAM_PARTICIPANT (submitted_at)
FOR EACH ROW
BEGIN
  DECLARE missing_answers INT;
...
CREATE TRIGGER automatic_grading 
